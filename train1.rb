...
class Train
  ...
  def self.find(num) # Дописан метод
    @@trains.detect { |train| train.number == num }
  end

  def initialize(number, number_cars) #!!! Вот тут я охренел от стольких inicialize'ов !!! ## Убрала я этот жуть
    @number = number
    ...
    while i <  @number_cars.to_i
      attach_car  #  Прицепляеся заданное количество вагонов 
      i +=1
    end
  end

  def block_trains (&block) #У класса Train написать метод, который принимает блок и проходит по всем вагонам поезда, передавая каждый объект вагона в блок.
    if block_given?
...
#!!! решение с блоком
  ...
  #  inicialize1(number, number_cars)
  #  if block_given?
  #    yield
  #  else
  #     @cars = @cars.map { |_volum| Car.new }
  #  end
  #  inicialize2
  #end
  #
  #в дочернем классе
  #def initialize(number, number_cars)
  #  super number, number_cars, { @cars.map! { |_volum| PassengerCar.new} }
  #end
  ...
  def car # сделала отдельно метод для создания нужного вагона
    @car =  Car.new
  end
  ...
  def validate! # Проверка формата номера поезда и количества вагонов
...
class PassengerTrain < Train
  ...
  include InstanceCounter # 5 Усложненное
  include Valid #!!! Достаточно включить его один раз в родительском классе, ведь никаких переменных класса в нем нет
